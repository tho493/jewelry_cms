...
<div style="max-width:600px">
    <div class="card">
        <div class="card-header"><h3>Sửa: {{ $category->name }}</h3></div>
        <div class="card-body">
            <form method="POST" action="{{ route('admin.categories.update', $category) }}">
                ...
                <div style="display:flex;gap:10px">
                    ...
                    <form method="POST" action="{{ route('admin.categories.destroy', $category) }}" onsubmit="return confirm('Xóa danh mục?')" style="flex:1">
                        @csrf @method('DELETE')
                        <button type="submit" class="btn btn-danger" style="width:100%;justify-content:center">🗑 Xóa</button>
                    </form>
                </div>
            </form>
        </div>
