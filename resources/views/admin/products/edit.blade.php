...
@section('topbar-actions')
    <a href="{{ route('products.show', $product->slug) }}" target="_blank" class="btn btn-secondary btn-sm">&#128065; Xem</a>
...
<style>
    ...
    #upload-toast.toast-success { background: rgba(76,175,125,0.95); color: #fff; }
    #upload-toast.toast-error   { background: rgba(224,82,82,0.95);  color: #fff; }
    ...
    .upload-zone .zone-icon { font-size: 28px; margin-bottom: 6px; display: block; }
    ...
    .upload-btn-group { display: flex; gap: 8px; flex-wrap: wrap; margin-bottom: 14px; }
</style>
@endpush
            ...
            <div class="card" x-data="mediaManager({{ $product->id }})">
                ...
                <div class="card-body">
                    ...
                    <div class="upload-btn-group">
                        <label class="btn btn-secondary btn-sm" style="cursor:pointer" title="Hỗ trợ: JPG, PNG, WEBP, GIF">
                            &#128444; Ảnh
                            <input type="file" multiple accept="image/jpeg,image/png,image/webp,image/gif"
                                   @change="uploadFiles($event)" style="display:none">
                        </label>
                        <label class="btn btn-secondary btn-sm" style="cursor:pointer" title="Hỗ trợ: MP4, MOV, AVI, WEBM">
                            &#127909; Video
                    ...
                    <div x-show="uploadError" x-cloak
                        ...
                        <button type="button" @click="uploadError=''" style="margin-left:auto;background:none;border:none;color:var(--danger);cursor:pointer">&#10005;</button>
                    </div>
                        ...
                        <template x-for="item in newMedia" :key="item.id">
                            <div class="media-item"
                                ...
                                <div x-show="item.type === 'video'"
                                     style="height:110px;display:flex;align-items:center;justify-content:center;font-size:36px">&#127909;</div>
                                <div x-show="item.type === 'audio'"
                                     style="height:110px;display:flex;align-items:center;justify-content:center;font-size:36px">&#127925;</div>

                                <div style="padding:6px 8px;display:flex;gap:4px">
                                    <button type="button"
                                            @click="setCoverNew(item)"
                                            class="btn btn-sm"
                                            x-show="item.type === 'image'"
                                            style="flex:1;padding:4px;font-size:11px;
            ...
            try {
                for (let i = 0; i < files.length; i++) {
                    ...
                    try {
                        ...
                        this.uploadError = `Lỗi mạng khi tải "${files[i].name}"`;
